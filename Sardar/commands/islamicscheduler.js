 ...
 * (sirf un groups ko jahan islamicpost ON ho)
 */
...
function setupIslamicScheduler(getApi, config, scheduledTasks) {
  ...
  const task = cron.schedule('0 * * * *', async () => {
    ...
    try {
      ...
      const targets = allThreads.filter(t => t?.settings?.islamicpost === true);
      ...
      logs.info('ISLAMIC_SCHEDULER', `Hourly Islamic messages sent to ${targets.length} groups`);
    } catch (e) {
...
module.exports = {
  config: {
    ...
    aliases: ['islamicpost'],
    description: 'Hourly Islamic / motivational messages group mein bhejta hai',
  ...
  async run({ api, event, args, send, config }) {
    ...
    if (action === 'on') {
      await ThreadsModel.updateSettings(threadID, { islamicpost: true });
      return send.reply(
        `╭─── « ☪️ ISLAMIC SCHEDULER » ───⟡\n` +
        `│\n` +
    ...
    if (action === 'off') {
      await ThreadsModel.updateSettings(threadID, { islamicpost: false });
      return send.reply(
        `╭─── « ☪️ ISLAMIC SCHEDULER » ───⟡\n` +
        `│\n` +
    ...
    const status = threadData?.settings?.islamicpost ? '✅ ON' : '❌ OFF';
    return send.reply(
      `╭─── « ☪️ ISLAMIC SCHEDULER » ───⟡\n` +
      `│\n` +
      `│ 📊 Status  : ${status}\n` +
